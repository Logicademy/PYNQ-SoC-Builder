...
class MarkdownWindow(ctk.CTkToplevel):
    def __init__(self, app):
        ...
        self.markdown_text = ""
        try:
            with open(self.app.path_to_markdown) as md_file:
                self.markdown_text = md_file.read()
        except:
            print(f"Could not load markdown menu - Error with file {self.app.path_to_markdown}")
            self.destroy()  # Destroy toplevel?
        
        html_content = md.markdown(self.markdown_text)
